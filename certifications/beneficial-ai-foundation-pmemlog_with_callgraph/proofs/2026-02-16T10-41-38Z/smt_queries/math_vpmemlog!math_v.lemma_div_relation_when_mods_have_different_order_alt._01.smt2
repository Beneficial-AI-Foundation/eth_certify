...
;; Function-Def pmemlog::math_v::lemma_div_relation_when_mods_have_different_order_alt
;; src/math_v.rs:131:5: 131:95 (#0)
       ...
       (=>
        ...
        (=>
         ...
         (=>
          (= tmp%2 (EucDiv x! d!))
          (=>
           (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%2 d!)
           (or
            (and
             (=>
              (> (EucDiv y! d!) (Add (EucDiv x! d!) 1))
              (=>
               (= tmp%3 (Sub (EucDiv y! d!) (EucDiv x! d!)))
               (and
                (=>
                 %%location_label%%2
                 (req%vstd!arithmetic.mul.lemma_mul_inequality. 2 tmp%3 d!)
                )
                (=>
                 (ens%vstd!arithmetic.mul.lemma_mul_inequality. 2 tmp%3 d!)
                 (=>
                  (= tmp%4 (EucDiv y! d!))
                  (=>
                   (= tmp%5 (EucDiv x! d!))
                   (=>
                    (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_sub_other_way. d! tmp%4 tmp%5)
                    (=>
