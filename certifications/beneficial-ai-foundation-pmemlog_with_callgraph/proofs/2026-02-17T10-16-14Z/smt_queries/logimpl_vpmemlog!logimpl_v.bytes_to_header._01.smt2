...
;; src/logimpl_v.rs:212:10: 212:68 (#0)
...
;; Function-Def pmemlog::logimpl_v::bytes_to_header
;; src/logimpl_v.rs:212:10: 212:68 (#0)
...
(push)
 ...
 (declare-const tmp%9 pmemlog!logimpl_v.PersistentHeaderMetadata.)
 ...
 (assert
  (not (=>
    (= tmp%1 bytes!)
    (=>
     (= tmp%2 (uClip SZ pmemlog!logimpl_v.header_crc_offset.?))
     (and
      (=>
       %%location_label%%0
       (uInv 64 (Add pmemlog!logimpl_v.header_crc_offset.? 8))
      )
      (=>
       (uInv 64 (Add pmemlog!logimpl_v.header_crc_offset.? 8))
       (=>
        (= tmp%4 (uClip SZ (uClip 64 (Add pmemlog!logimpl_v.header_crc_offset.? 8))))
        (and
         (=>
          %%location_label%%1
         ...
         (=>
          ...
          (=>
           (= crc_bytes@ (%Poly%slice%<u8.>. tmp%3))
           (=>
            (= tmp%6 (uClip SZ pmemlog!logimpl_v.header_head_offset.?))
            (=>
             (= tmp%7 (uClip SZ pmemlog!logimpl_v.header_size.?))
             (and
              (=>
               %%location_label%%2
               (req%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%6 tmp%7)
              )
              (=>
               (ens%vstd!slice.slice_subrange. $ (UINT 8) (Poly%slice%<u8.>. bytes!) tmp%6 tmp%7
                tmp%5
               )
               (=>
                (= metadata_bytes@ (%Poly%slice%<u8.>. tmp%5))
                (and
                 (=>
                  %%location_label%%3
                 ...
                 (=>
                  (ens%vstd!bytes.u64_from_le_bytes. crc_bytes@ tmp%8)
                  (=>
                   (= tmp%10 tmp%8)
                   (and
                    (=>
                     %%location_label%%4
                     (req%pmemlog!logimpl_v.bytes_to_metadata. metadata_bytes@)
                    )
                    (=>
                     (ens%pmemlog!logimpl_v.bytes_to_metadata. metadata_bytes@ tmp%9)
                     (=>
                      (= out! (pmemlog!logimpl_v.PersistentHeader./PersistentHeader (%I (I tmp%10)) (%Poly%pmemlog!logimpl_v.PersistentHeaderMetadata.
                         (Poly%pmemlog!logimpl_v.PersistentHeaderMetadata. tmp%9)
                      )))
                      (=>
                       %%location_label%%5
