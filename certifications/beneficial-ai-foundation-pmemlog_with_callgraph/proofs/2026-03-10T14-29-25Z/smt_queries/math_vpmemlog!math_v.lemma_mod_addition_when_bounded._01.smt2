...
;; Function-Def pmemlog::math_v::lemma_mod_addition_when_bounded
;; src/math_v.rs:230:5: 230:73 (#0)
  ...
  (not (and
    ...
    (=>
     ...
     (=>
      (= tmp%1 (EucDiv x! d!))
      (=>
       (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%1 d!)
       (=>
        (= tmp%2 (Add x! y!))
        (=>
         (= tmp%3 (EucDiv x! d!))
         (=>
          (= tmp%4 (Add (EucMod x! d!) y!))
          (and
           (=>
            %%location_label%%1
            (req%vstd!arithmetic.div_mod.lemma_fundamental_div_mod_converse. tmp%2 d! tmp%3 tmp%4)
           )
           (=>
            (ens%vstd!arithmetic.div_mod.lemma_fundamental_div_mod_converse. tmp%2 d! tmp%3 tmp%4)
            (=>
             %%location_label%%2
             (= (EucMod (Add x! y!) d!) (Add (EucMod x! d!) y!))
 ))))))))))))
 (get-info :version)
