...
;; Function-Def pmemlog::main_t::InfiniteLogImpl::start
;; src/main_t.rs:123:9: 123:108 (#0)
...
(push)
 ...
 (assert
  (forall ((%%hole%%0 Poly) (s1$ Poly) (s2$ Poly)) (!
    (= (%%apply%%1 (%%lambda%%1 %%hole%%0) s1$ s2$) %%hole%%0)
 ...
 (assert
  (not (and
    (=>
     %%location_label%%0
     (req%pmemlog!pmemspec_t.impl&%0.new. $ TYPE%pmemlog!main_t.TrustedPermission. PM&.
      PM& pm!
    ))
    (=>
     (ens%pmemlog!pmemspec_t.impl&%0.new. $ TYPE%pmemlog!main_t.TrustedPermission. PM&.
      PM& pm! wrpm@0
     )
     (=>
      (= tmp%1 (%Poly%vstd!seq.Seq<u8.>. (pmemlog!pmemspec_t.PersistentMemory.view.? PM&.
         PM& pm!
      )))
      (=>
       (= tmp%2 (mk_fun (%%lambda%%1 (B false))))
       (=>
        ...
        (=>
         ...
         (=>
          ...
          (and
           (=>
            %%location_label%%1
            (req%pmemlog!logimpl_v.impl&%0.untrusted_start. $ TYPE%pmemlog!main_t.TrustedPermission.
             PM&. PM& wrpm@0 device_size! (Poly%pmemlog!main_t.TrustedPermission. perm@1)
           ))
           (=>
            (has_type wrpm@1 (TYPE%pmemlog!pmemspec_t.WriteRestrictedPersistentMemory. $ TYPE%pmemlog!main_t.TrustedPermission.
              PM&. PM&
            ))
            (=>
             (ens%pmemlog!logimpl_v.impl&%0.untrusted_start. $ TYPE%pmemlog!main_t.TrustedPermission.
              PM&. PM& wrpm@0 wrpm@1 device_size! (Poly%pmemlog!main_t.TrustedPermission. perm@1)
              tmp%%@
             )
             (=>
              (= result! (ite
                (is-core!result.Result./Ok tmp%%@)
                (let
                 ((untrusted_log_impl$ (%Poly%pmemlog!logimpl_v.UntrustedLogImpl. (core!result.Result./Ok/0
                     $ TYPE%pmemlog!logimpl_v.UntrustedLogImpl. $ TYPE%pmemlog!main_t.InfiniteLogErr.
                     (%Poly%core!result.Result. (Poly%core!result.Result. tmp%%@))
                 ))))
                 (core!result.Result./Ok (Poly%pmemlog!main_t.InfiniteLogImpl. (pmemlog!main_t.InfiniteLogImpl./InfiniteLogImpl
                ...
                (let
                 ((e$ (%Poly%pmemlog!main_t.InfiniteLogErr. (core!result.Result./Err/0 $ TYPE%pmemlog!logimpl_v.UntrustedLogImpl.
                     $ TYPE%pmemlog!main_t.InfiniteLogErr. (%Poly%core!result.Result. (Poly%core!result.Result.
                       tmp%%@
                 ))))))
                 (core!result.Result./Err (Poly%pmemlog!main_t.InfiniteLogErr. e$))
              )))
              (=>
               %%location_label%%2
               (ite
                 ...
                 (and
                  (and
                   ...
                   (ext_eq false (TYPE%core!option.Option. $ TYPE%pmemlog!infinitelog_t.AbstractInfiniteLogState.)
                    ...
                    ) (%%apply%%0 (pmemlog!main_t.recovery_view.? (I 0)) (pmemlog!pmemspec_t.PersistentMemory.view.?
                      PM&. PM& pm!
                  ))))
                  (= (pmemlog!main_t.impl&%4.constants.? PM&. PM& (Poly%pmemlog!main_t.InfiniteLogImpl.
                     trusted_log_impl$
