...
;; src/logimpl_v.rs:380:5: 382:44 (#0)
...
;; Function-Def pmemlog::logimpl_v::lemma_data_write_is_safe
;; src/logimpl_v.rs:380:5: 382:44 (#0)
 ...
 (assert
  (not (=>
    ...
    (and
     (=>
      %%location_label%%0
      (req%pmemlog!logimpl_v.lemma_append_data_update_view. pm! bytes! write_addr!)
     )
     (=>
      (ens%pmemlog!logimpl_v.lemma_append_data_update_view. pm! bytes! write_addr!)
      (and
       (=>
        %%location_label%%1
        (req%pmemlog!logimpl_v.lemma_same_log_state. pm! new_pm@)
       )
       (=>
        (ens%pmemlog!logimpl_v.lemma_same_log_state. pm! new_pm@)
        (and
         (=>
          (has_type chunks_flushed@ (TYPE%vstd!set.Set. $ INT))
          (=>
           ...
           (and
            (=>
             %%location_label%%2
             (req%pmemlog!logimpl_v.lemma_append_data_update_view_crash. pm! bytes! write_addr!
              (%Poly%vstd!set.Set<int.>. chunks_flushed@)
            ))
            (=>
             (ens%pmemlog!logimpl_v.lemma_append_data_update_view_crash. pm! bytes! write_addr!
              (%Poly%vstd!set.Set<int.>. chunks_flushed@)
             )
             (and
              (=>
               %%location_label%%3
               (req%pmemlog!logimpl_v.lemma_same_log_state. pm! new_pm$1@)
              )
              (=>
               (ens%pmemlog!logimpl_v.lemma_same_log_state. pm! new_pm$1@)
               (and
                (=>
                 %%location_label%%4
                 (req%pmemlog!logimpl_v.lemma_same_permissions. Perm&. Perm& pm! new_pm$1@ perm!)
                )
                (=>
                 (ens%pmemlog!logimpl_v.lemma_same_permissions. Perm&. Perm& pm! new_pm$1@ perm!)
                 (=>
                  %%location_label%%5
                  (%B (let
         ...
         (=>
          (forall ((chunks_flushed$ Poly)) (!
            ...
            :qid user_pmemlog__logimpl_v__lemma_data_write_is_safe_48
            :skolemid skolem_user_pmemlog__logimpl_v__lemma_data_write_is_safe_48
          ))
          (and
           (=>
            %%location_label%%6
           ...
           (and
            (=>
             ...
             (forall ((chunks_flushed$ Poly)) (!
               ...
               :qid user_pmemlog__logimpl_v__lemma_data_write_is_safe_47
               :skolemid skolem_user_pmemlog__logimpl_v__lemma_data_write_is_safe_47
            )))
            (and
             (=>
              %%location_label%%8
