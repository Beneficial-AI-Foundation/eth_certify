...
;; src/logimpl_v.rs:1285:9: 1285:118 (#0)
...
;; Function-Def pmemlog::logimpl_v::UntrustedLogImpl::read_incorruptible_boolean
;; src/logimpl_v.rs:1285:9: 1285:118 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= tmp%2 (I pmemlog!logimpl_v.incorruptible_bool_pos.?))
    (and
     (=>
      %%location_label%%0
      (req%pmemlog!pmemspec_t.PersistentMemory.read. PM&. PM& pm! tmp%2 (I 8))
     )
     (=>
      (ens%pmemlog!pmemspec_t.PersistentMemory.read. PM&. PM& pm! tmp%2 (I 8) tmp%1)
      (=>
       (= bytes@ (%Poly%alloc!vec.Vec<u8./allocator_global%.>. tmp%1))
       (=>
        ...
        (and
         (=>
          %%location_label%%1
          (req%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. tmp%3))
         )
         (=>
          (ens%vstd!bytes.u64_from_le_bytes. (%Poly%slice%<u8.>. tmp%3) ib@)
          (=>
           (= verus_tmp@1 (%Poly%vstd!seq.Seq<int.>. (vstd!seq.Seq.new.? $ INT $ (TYPE%fun%1. $
               INT $ INT
              ) (I 8) (Poly%fun%1. (mk_fun (%%lambda%%0 pmemlog!logimpl_v.incorruptible_bool_pos.?)))
           )))
           (=>
            (= verus_tmp_addrs@ verus_tmp@1)
            (=>
                  ...
                  (=>
                   ...
                   (or
                    (and
                     (=>
                      ...
                      (and
                       (=>
                        %%location_label%%2
                        (req%pmemlog!pmemspec_t.axiom_corruption_detecting_boolean. ib@ spec_ib@ addrs@1)
                       )
                       (=>
                        (ens%pmemlog!pmemspec_t.axiom_corruption_detecting_boolean. ib@ spec_ib@ addrs@1)
                        %%switch_label%%1
                     )))
                     (=>
                    ...
                    (and
                     (not %%switch_label%%1)
                     (=>
                      (= tmp%4 (core!result.Result./Ok (I ib@)))
                      %%switch_label%%0
               )))))))
               (=>
                (not (or
                ...
                (=>
                 (= tmp%4 (core!result.Result./Err (Poly%pmemlog!main_t.InfiniteLogErr. pmemlog!main_t.InfiniteLogErr./CRCMismatch)))
                 %%switch_label%%0
              )))
              (and
               (not %%switch_label%%0)
