...
;; Function-Def pmemlog::main_t::TrustedPermission::new
;; src/main_t.rs:51:15: 52:44 (#0)
(get-info :all-statistics)
(push)
 (declare-const perm! pmemlog!main_t.TrustedPermission.)
 ...
 (assert
  (has_type (Poly%fun%2. next!) (TYPE%fun%2. $ TYPE%pmemlog!infinitelog_t.AbstractInfiniteLogState.
    $ TYPE%pmemlog!infinitelog_t.AbstractInfiniteLogState. $ BOOL
 )))
 ...
 (assert
  (forall ((%%hole%%0 Dcr) (%%hole%%1 Type) (%%hole%%2 Dcr) (%%hole%%3 Type) (%%hole%%4
     Poly
    ) (%%hole%%5 Poly) (%%hole%%6 Poly) (s$ Poly)
   ) (!
    (= (%%apply%%0 (%%lambda%%1 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5
       %%hole%%6
      ) s$
     ) (B (pmemlog!sccf.is_state_allowable.? %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4
       s$ %%hole%%5 %%hole%%6
    )))
 ...
 (assert
  (not (=>
    (= perm! (pmemlog!main_t.TrustedPermission./TrustedPermission (%Poly%fun%1. (Poly%fun%1.
        (mk_fun (%%lambda%%1 $ (TYPE%vstd!seq.Seq. $ (UINT 8)) $ TYPE%pmemlog!infinitelog_t.AbstractInfiniteLogState.
          (Poly%vstd!seq.Seq<u8.>. cur!) (Poly%fun%1. (pmemlog!main_t.recovery_view.? (I 0)))
          (Poly%fun%2. next!)
    ))))))
    (=>
     %%location_label%%0
     (forall ((s$ Poly)) (!
       ...
       :qid user_pmemlog__main_t__TrustedPermission__new_30
       :skolemid skolem_user_pmemlog__main_t__TrustedPermission__new_30
 ))))))
 (get-info :version)
