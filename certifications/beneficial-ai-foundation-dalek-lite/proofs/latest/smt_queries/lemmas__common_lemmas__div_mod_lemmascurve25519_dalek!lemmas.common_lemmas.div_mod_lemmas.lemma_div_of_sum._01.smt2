...
;; curve25519-dalek/src/lemmas/common_lemmas/div_mod_lemmas.rs:91:1: 91:54 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::div_mod_lemmas::lemma_div_of_sum
;; curve25519-dalek/src/lemmas/common_lemmas/div_mod_lemmas.rs:91:1: 91:54 (#0)
    ...
    (=>
     ...
     (and
      (and
       (=>
        %%location_label%%0
        (req%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. a! k!)
       )
       (=>
        (ens%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. a! k!)
        (=>
         %%location_label%%1
         (= a! (nClip (Add (nClip (Mul k! a0@)) (EucMod a! k!))))
      )))
      (=>
       (= a! (nClip (Add (nClip (Mul k! a0@)) (EucMod a! k!))))
       (and
        (and
         (=>
          %%location_label%%2
          (req%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. b! k!)
         )
         (=>
          (ens%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. b! k!)
          (=>
           %%location_label%%3
           (= b! (nClip (Add (nClip (Mul k! b0@)) (EucMod b! k!))))
        )))
        (=>
         (= b! (nClip (Add (nClip (Mul k! b0@)) (EucMod b! k!))))
         (and
          (=>
           (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add. k! a0@ b0@)
           (=>
            %%location_label%%4
            (= (nClip (Add a! b!)) (nClip (Add (nClip (Add (nClip (Mul k! (nClip (Add a0@ b0@))))
                 (EucMod a! k!)
                )
               ) (EucMod b! k!)
          )))))
          (=>
           ...
           (=>
            (= tmp%1 (nClip (Add a0@ b0@)))
            (=>
             (= tmp%2 (nClip (Add (EucMod a! k!) (EucMod b! k!))))
             (and
              (=>
               %%location_label%%5
               (req%vstd!arithmetic.div_mod.lemma_div_multiples_vanish_fancy. tmp%1 tmp%2 k!)
              )
              (=>
               (ens%vstd!arithmetic.div_mod.lemma_div_multiples_vanish_fancy. tmp%1 tmp%2 k!)
               (=>
                %%location_label%%6
                (= (EucDiv (nClip (Add a! b!)) k!) (nClip (Add (EucDiv a! k!) (EucDiv b! k!))))
 )))))))))))))))
