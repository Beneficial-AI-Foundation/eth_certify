...
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:168:1: 178:2 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::mul_lemmas::lemma_mul_distributive_8_terms
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:168:1: 178:2 (#0)
...
(push)
 ...
 (assert
  (not (and
    (=>
     (= tmp%1 (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!))
     (=>
      (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. n! tmp%1)
      (=>
       %%location_label%%0
       (= (Mul n! (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!))
        (Mul (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!) n!)
    ))))
    (=>
     (= (Mul n! (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!))
      (Mul (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!) n!)
     ...
     (and
      (=>
       (= tmp%2 (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!))
       (=>
        (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add. n! tmp%2 x8!)
        (=>
         %%location_label%%1
         (= (Mul n! (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!))
          (Add (Mul n! (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!)) (Mul
            n! x8!
      ))))))
      (=>
       (= (Mul n! (Add (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!) x8!))
       ...
       (and
        (=>
         ...
         (=>
          %%location_label%%2
          (= (Mul n! (Add (Add (Add (Add (Add (Add x1! x2!) x3!) x4!) x5!) x6!) x7!)) (Add (Add
             (Add (Add (Add (Add (Mul n! x1!) (Mul n! x2!)) (Mul n! x3!)) (Mul n! x4!)) (Mul n!
               x5!
              )
             ) (Mul n! x6!)
            ) (Mul n! x7!)
