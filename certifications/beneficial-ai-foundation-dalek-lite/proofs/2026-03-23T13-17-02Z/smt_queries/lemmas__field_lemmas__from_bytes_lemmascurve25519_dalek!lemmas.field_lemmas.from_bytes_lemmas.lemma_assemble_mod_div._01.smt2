...
;; curve25519-dalek/src/lemmas/field_lemmas/from_bytes_lemmas.rs:22:1: 22:60 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::from_bytes_lemmas::lemma_assemble_mod_div
;; curve25519-dalek/src/lemmas/field_lemmas/from_bytes_lemmas.rs:22:1: 22:60 (#0)
...
(push)
 ...
 (declare-const pd@ Int)
 (declare-const pbd@ Int)
 (declare-const amod@ Int)
 (declare-const adiv@ Int)
 (assert
  fuel_defaults
 )
 (assert
 ...
 (assert
  (not (=>
    (= pb@ (vstd!arithmetic.power2.pow2.? (I b!)))
    (=>
     (= pd@ (vstd!arithmetic.power2.pow2.? (I d!)))
     (=>
      (= pbd@ (vstd!arithmetic.power2.pow2.? (I (nClip (Add b! d!)))))
      (and
       (=>
        (ens%vstd!arithmetic.power2.lemma_pow2_adds. b! d!)
        (=>
         %%location_label%%0
         (= pbd@ (nClip (Mul pb@ pd@)))
       ))
       (=>
        ...
        (=>
         ...
         (=>
          ...
          (and
           (=>
            (ens%vstd!arithmetic.mul.lemma_mul_is_associative. pb@ pd@ adiv@)
            (=>
             (= tmp%1 (nClip (Mul pd@ adiv@)))
             (=>
              (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. pb@ tmp%1)
              (=>
               %%location_label%%1
               (= (nClip (Mul pbd@ adiv@)) (nClip (Mul (nClip (Mul pd@ adiv@)) pb@)))
           ))))
           (=>
            (= (nClip (Mul pbd@ adiv@)) (nClip (Mul (nClip (Mul pd@ adiv@)) pb@)))
            (and
             (=>
              (= tmp%2 (nClip (Mul pd@ adiv@)))
              (=>
               (ens%curve25519_dalek!lemmas.common_lemmas.mul_lemmas.lemma_nat_distributive. pb@
                amod@ tmp%2
               )
               (=>
                %%location_label%%2
                (= (nClip (Mul (nClip (Add amod@ (nClip (Mul pd@ adiv@)))) pb@)) (nClip (Add (nClip (
                     Mul amod@ pb@
                    )
                   ) (nClip (Mul (nClip (Mul pd@ adiv@)) pb@))
             ))))))
             (=>
              (= (nClip (Mul (nClip (Add amod@ (nClip (Mul pd@ adiv@)))) pb@)) (nClip (Add (nClip (
                   Mul amod@ pb@
               ...
               (and
                ...
                (=>
                 ...
                 (and
                  (=>
                   %%location_label%%4
                   (req%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. a! pd@)
                  )
                  (=>
                   (ens%vstd!arithmetic.div_mod.lemma_fundamental_div_mod. a! pd@)
                   (=>
                    %%location_label%%5
                    (= (nClip (Add amod@ (nClip (Mul pd@ adiv@)))) a!)
               )))))
               (=>
                (= (nClip (Add amod@ (nClip (Mul pd@ adiv@)))) a!)
                (=>
