...
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:1248:1: 1248:75 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::field_algebra_lemmas::lemma_product_of_squares_eq_square_of_product
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:1248:1: 1248:75 (#0)
     ...
     (=>
      ...
      (=>
       ...
       (=>
        ...
        (=>
         (= xy2@ (curve25519_dalek!specs.field_specs.field_square.? (I xy@)))
         (and
          (=>
           (= tmp%1 (nClip (Mul x! x!)))
           (=>
            ...
            (and
             (=>
              %%location_label%%0
              (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop. tmp%1 tmp%2 p@)
             )
             (=>
              (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop. tmp%1 tmp%2 p@)
              (=>
               (= tmp%3 (nClip (Mul x! y!)))
               (=>
                (= tmp%4 (nClip (Mul x! y!)))
                (and
                 (=>
                  %%location_label%%1
                  (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop. tmp%3 tmp%4 p@)
                 )
                 (=>
                  (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop. tmp%3 tmp%4 p@)
                  (and
                   (=>
                    (= tmp%5 (nClip (Mul x! y!)))
                    (=>
                     (ens%vstd!arithmetic.mul.lemma_mul_is_associative. x! y! tmp%5)
                     (=>
                      (ens%vstd!arithmetic.mul.lemma_mul_is_associative. y! x! y!)
                      (=>
                       (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. y! x!)
                       (=>
                        (ens%vstd!arithmetic.mul.lemma_mul_is_associative. x! y! y!)
                        (=>
                         (= tmp%6 (nClip (Mul y! y!)))
                         (=>
                          (ens%vstd!arithmetic.mul.lemma_mul_is_associative. x! x! tmp%6)
                          (=>
                           %%location_label%%2
                           (= (nClip (Mul (nClip (Mul x! y!)) (nClip (Mul x! y!)))) (nClip (Mul (nClip (Mul x! x!))
                              (nClip (Mul y! y!))
                   )))))))))))
                   (=>
                    (= (nClip (Mul (nClip (Mul x! y!)) (nClip (Mul x! y!)))) (nClip (Mul (nClip (Mul x! x!))
                    ...
                    (=>
                     %%location_label%%3
                     (= (curve25519_dalek!specs.field_specs.field_mul.? (I x2@) (I y2@)) xy2@)
          )))))))))))
          (=>
           (= (curve25519_dalek!specs.field_specs.field_mul.? (I x2@) (I y2@)) xy2@)
           (=>
