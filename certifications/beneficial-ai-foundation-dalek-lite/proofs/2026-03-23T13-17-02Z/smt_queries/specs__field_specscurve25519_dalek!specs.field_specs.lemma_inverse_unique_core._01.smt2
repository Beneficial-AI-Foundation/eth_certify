...
;; curve25519-dalek/src/specs/field_specs.rs:185:7: 185:67 (#0)
...
;; Function-Def curve25519_dalek::specs::field_specs::lemma_inverse_unique_core
;; curve25519-dalek/src/specs/field_specs.rs:185:7: 185:67 (#0)
 ...
 (assert
  (not (and
    (and
     (=>
      %%location_label%%0
      (req%vstd!arithmetic.div_mod.lemma_small_mod. w! p!)
     )
     (=>
      (ens%vstd!arithmetic.div_mod.lemma_small_mod. w! p!)
      (=>
       %%location_label%%1
       (= (EucMod w! p!) w!)
    )))
    (=>
     ...
     (and
      (and
       (=>
        %%location_label%%2
        (req%vstd!arithmetic.div_mod.lemma_small_mod. z! p!)
       )
       (=>
        (ens%vstd!arithmetic.div_mod.lemma_small_mod. z! p!)
        (=>
         %%location_label%%3
         (= (EucMod z! p!) z!)
      )))
       ...
       (and
        (=>
         ...
         (and
          (=>
           %%location_label%%4
           (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop_general. tmp%1 z! p!)
          )
          (=>
           (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop_general. tmp%1 z! p!)
           (=>
            (= tmp%2 (= (EucMod (nClip (Mul (EucMod (nClip (Mul a! w!)) p!) z!)) p!) (EucMod (nClip
                (Mul (nClip (Mul a! w!)) z!)
               ) p!
            )))
            (and
             ...
             (=>
              tmp%2
              (=>
               (= tmp%3 (= (EucMod (nClip (Mul (EucMod (nClip (Mul a! w!)) p!) z!)) p!) (EucMod (nClip
                   (Mul 1 z!)
                  ) p!
               )))
               (and
                ...
                (=>
                 tmp%3
                 (=>
                  (= tmp%4 (= (EucMod (nClip (Mul 1 z!)) p!) z!))
                  (and
                   (=>
                    %%location_label%%7
                    tmp%4
                   ...
                   (=>
                    ...
                    (=>
                     %%location_label%%8
                     (= (EucMod (nClip (Mul (nClip (Mul a! w!)) z!)) p!) z!)
        )))))))))))))
        (=>
         (= (EucMod (nClip (Mul (nClip (Mul a! w!)) z!)) p!) z!)
         (and
          (=>
           ...
           (and
            (=>
             %%location_label%%9
             (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop_general. tmp%5 w! p!)
            )
            (=>
             (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop_general. tmp%5 w! p!)
             (=>
              (= tmp%6 (= (EucMod (nClip (Mul (EucMod (nClip (Mul a! z!)) p!) w!)) p!) (EucMod (nClip
                  (Mul (nClip (Mul a! z!)) w!)
                 ) p!
              )))
              (and
               ...
               (=>
                tmp%6
                (=>
                 (= tmp%7 (= (EucMod (nClip (Mul (EucMod (nClip (Mul a! z!)) p!) w!)) p!) (EucMod (nClip
                     (Mul 1 w!)
                    ) p!
                 )))
                 (and
                  ...
                  (=>
                   tmp%7
                   (=>
                    (= tmp%8 (= (EucMod (nClip (Mul 1 w!)) p!) w!))
                    (and
                     (=>
                      %%location_label%%12
                      tmp%8
                     ...
                     (=>
                      ...
                      (=>
                       %%location_label%%13
                       (= (EucMod (nClip (Mul (nClip (Mul a! z!)) w!)) p!) w!)
          )))))))))))))
          (=>
           (= (EucMod (nClip (Mul (nClip (Mul a! z!)) w!)) p!) w!)
           (and
            (=>
             (ens%vstd!arithmetic.mul.lemma_mul_is_associative. a! w! z!)
             (=>
              (ens%vstd!arithmetic.mul.lemma_mul_is_associative. a! z! w!)
              (=>
               (= tmp%9 (= (nClip (Mul (nClip (Mul a! w!)) z!)) (nClip (Mul a! (nClip (Mul w! z!))))))
               (and
                (=>
                 %%location_label%%14
                 tmp%9
                ...
                (=>
                 tmp%9
                 (=>
                  (= tmp%10 (= (nClip (Mul w! z!)) (nClip (Mul z! w!))))
                  (and
                   (=>
                    %%location_label%%15
                    tmp%10
                   ...
                   (=>
                    tmp%10
                    (=>
                     (= tmp%11 (= (nClip (Mul a! (nClip (Mul w! z!)))) (nClip (Mul a! (nClip (Mul z! w!))))))
                     (and
                      (=>
                       %%location_label%%16
                       tmp%11
                      ...
                      (=>
                       tmp%11
                       (=>
                        (= tmp%12 (= (nClip (Mul a! (nClip (Mul z! w!)))) (nClip (Mul (nClip (Mul a! z!)) w!))))
                        (and
                         (=>
                          %%location_label%%17
                          tmp%12
                         ...
                         (=>
                          ...
                          (=>
                           %%location_label%%18
                           (= (nClip (Mul (nClip (Mul a! w!)) z!)) (nClip (Mul (nClip (Mul a! z!)) w!)))
            )))))))))))))))
            (=>
             (= (nClip (Mul (nClip (Mul a! w!)) z!)) (nClip (Mul (nClip (Mul a! z!)) w!)))
             (=>
