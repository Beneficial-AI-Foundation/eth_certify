...
;; curve25519-dalek/src/lemmas/field_lemmas/from_bytes_lemmas.rs:56:1: 56:70 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::from_bytes_lemmas::lemma_assemble_pow_a_pow
;; curve25519-dalek/src/lemmas/field_lemmas/from_bytes_lemmas.rs:56:1: 56:70 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= d@ (nClip (Sub (nClip (Mul k! 8)) l!)))
    (=>
     (= dd@ (nClip (Add (nClip (Mul j! 8)) l!)))
     (=>
      (= pjl@ (vstd!arithmetic.power2.pow2.? (I (nClip (Add (nClip (Mul j! 8)) l!)))))
      (and
       (=>
        (= tmp%1 (Mul a! (vstd!arithmetic.power2.pow2.? (I d@))))
        (=>
         (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. pjl@ tmp%1)
         (=>
          %%location_label%%0
          (= (nClip (Mul pjl@ (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))))) (nClip
            (Mul (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))) pjl@)
       )))))
       (=>
        (= (nClip (Mul pjl@ (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))))) (nClip
          (Mul (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))) pjl@)
        ...
        (and
         (=>
          (= tmp%2 (vstd!arithmetic.power2.pow2.? (I d@)))
          (=>
           (ens%vstd!arithmetic.mul.lemma_mul_is_associative. a! tmp%2 pjl@)
           (=>
            (ens%vstd!arithmetic.power2.lemma_pow2_adds. d@ dd@)
            (=>
             %%location_label%%1
             (= (nClip (Mul (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))) pjl@)) (nClip
               (Mul a! (vstd!arithmetic.power2.pow2.? (I (nClip (Add d@ dd@)))))
         ))))))
         (=>
          (= (nClip (Mul (nClip (Mul a! (vstd!arithmetic.power2.pow2.? (I d@)))) pjl@)) (nClip
            (Mul a! (vstd!arithmetic.power2.pow2.? (I (nClip (Add d@ dd@)))))
          ...
          (and
           (and
            (=>
             (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add_other_way. 8 d@ dd@)
             (=>
              %%location_label%%2
              (= (nClip (Mul (nClip (Add j! k!)) 8)) (nClip (Add (nClip (Mul j! 8)) (nClip (Mul k! 8)))))
            ))
            (=>
             (= (nClip (Mul (nClip (Add j! k!)) 8)) (nClip (Add (nClip (Mul j! 8)) (nClip (Mul k! 8)))))
             (=>
              %%location_label%%3
              (= (nClip (Add d@ dd@)) (nClip (Mul (nClip (Add j! k!)) 8)))
           )))
           (=>
            (= (nClip (Add d@ dd@)) (nClip (Mul (nClip (Add j! k!)) 8)))
            (=>
