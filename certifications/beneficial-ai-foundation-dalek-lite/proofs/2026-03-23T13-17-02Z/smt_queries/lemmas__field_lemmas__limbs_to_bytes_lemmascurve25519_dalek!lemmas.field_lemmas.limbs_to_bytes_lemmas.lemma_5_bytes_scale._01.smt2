...
;; curve25519-dalek/src/lemmas/field_lemmas/limbs_to_bytes_lemmas.rs:480:7: 488:2 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::limbs_to_bytes_lemmas::lemma_5_bytes_scale
;; curve25519-dalek/src/lemmas/field_lemmas/limbs_to_bytes_lemmas.rs:480:7: 488:2 (#0)
(get-info :all-statistics)
(push)
 (declare-const value! Int)
 (declare-const byte0! Int)
 ...
 (assert
  (not (=>
    (= sum@ (nClip (Add (nClip (Add (nClip (Add (nClip (Add (nClip (Mul byte0! (vstd!arithmetic.power2.pow2.?
                (I 0)
              ))
             ) (nClip (Mul byte1! (vstd!arithmetic.power2.pow2.? (I 8))))
            )
    ...
    (=>
     (= p@ (vstd!arithmetic.power2.pow2.? (I scale!)))
     (=>
      (= tmp%1 (= (nClip (Mul sum@ p@)) (nClip (Mul value! p@))))
      (and
       (=>
        %%location_label%%0
        tmp%1
       ...
       (=>
        tmp%1
        (=>
         (= t0@ (nClip (Mul byte0! (vstd!arithmetic.power2.pow2.? (I 0)))))
         (=>
          (= t1@ (nClip (Mul byte1! (vstd!arithmetic.power2.pow2.? (I 8)))))
          (=>
           (= t2@ (nClip (Mul byte2! (vstd!arithmetic.power2.pow2.? (I 16)))))
           (=>
            (= t3@ (nClip (Mul byte3! (vstd!arithmetic.power2.pow2.? (I 24)))))
            (=>
             (= t4@ (nClip (Mul byte4! (vstd!arithmetic.power2.pow2.? (I 32)))))
             (and
              (=>
               (ens%curve25519_dalek!lemmas.common_lemmas.mul_lemmas.lemma_mul_distributive_5_terms.
                p@ t0@ t1@ t2@ t3@ t4@
               ...
               (=>
                %%location_label%%1
                (= (nClip (Mul sum@ p@)) (nClip (Add (nClip (Add (nClip (Add (nClip (Add (nClip (Mul t0@ p@))
                         (nClip (Mul t1@ p@))
                        )
                       ) (nClip (Mul t2@ p@))
                      )
              ...
              (=>
               ...
               (and
                (=>
                 ...
                 (=>
                  (ens%vstd!arithmetic.mul.lemma_mul_is_associative. byte0! tmp%2 p@)
                  (=>
                   (ens%vstd!arithmetic.power2.lemma_pow2_adds. 0 scale!)
                   (=>
                    %%location_label%%2
                    (= (nClip (Mul t0@ p@)) (nClip (Mul byte0! (vstd!arithmetic.power2.pow2.? (I scale!)))))
                ))))
                (=>
                 (= (nClip (Mul t0@ p@)) (nClip (Mul byte0! (vstd!arithmetic.power2.pow2.? (I scale!)))))
                 (and
                  (=>
                   (= tmp%3 (vstd!arithmetic.power2.pow2.? (I 8)))
                   (=>
                    (ens%vstd!arithmetic.mul.lemma_mul_is_associative. byte1! tmp%3 p@)
                    (=>
                     (ens%vstd!arithmetic.power2.lemma_pow2_adds. 8 scale!)
                     (=>
                      (= tmp%4 (vstd!arithmetic.power2.pow2.? (I 8)))
                      (=>
                       (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%4 p@)
                       (=>
                        %%location_label%%3
                        (= (nClip (Mul t1@ p@)) (nClip (Mul byte1! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                               scale! 8
                  )))))))))))))
                  (=>
                   (= (nClip (Mul t1@ p@)) (nClip (Mul byte1! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                   ...
                   (and
                    (=>
                     (= tmp%5 (vstd!arithmetic.power2.pow2.? (I 16)))
                     (=>
                      (ens%vstd!arithmetic.mul.lemma_mul_is_associative. byte2! tmp%5 p@)
                      (=>
                       (ens%vstd!arithmetic.power2.lemma_pow2_adds. 16 scale!)
                       (=>
                        (= tmp%6 (vstd!arithmetic.power2.pow2.? (I 16)))
                        (=>
                         (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%6 p@)
                         (=>
                          %%location_label%%4
                          (= (nClip (Mul t2@ p@)) (nClip (Mul byte2! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                                 scale! 16
                    )))))))))))))
                    (=>
                     (= (nClip (Mul t2@ p@)) (nClip (Mul byte2! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                     ...
                     (and
                      (=>
                       (= tmp%7 (vstd!arithmetic.power2.pow2.? (I 24)))
                       (=>
                        (ens%vstd!arithmetic.mul.lemma_mul_is_associative. byte3! tmp%7 p@)
                        (=>
                         (ens%vstd!arithmetic.power2.lemma_pow2_adds. 24 scale!)
                         (=>
                          (= tmp%8 (vstd!arithmetic.power2.pow2.? (I 24)))
                          (=>
                           (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%8 p@)
                           (=>
                            %%location_label%%5
                            (= (nClip (Mul t3@ p@)) (nClip (Mul byte3! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                                   scale! 24
                      )))))))))))))
                      (=>
                       (= (nClip (Mul t3@ p@)) (nClip (Mul byte3! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                       ...
                       (and
                        (=>
                         (= tmp%9 (vstd!arithmetic.power2.pow2.? (I 32)))
                         (=>
                          (ens%vstd!arithmetic.mul.lemma_mul_is_associative. byte4! tmp%9 p@)
                          (=>
                           (ens%vstd!arithmetic.power2.lemma_pow2_adds. 32 scale!)
                           (=>
                            (= tmp%10 (vstd!arithmetic.power2.pow2.? (I 32)))
                            (=>
                             (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. tmp%10 p@)
                             (=>
                              %%location_label%%6
                              (= (nClip (Mul t4@ p@)) (nClip (Mul byte4! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
                                     scale! 32
                        )))))))))))))
                        (=>
                         (= (nClip (Mul t4@ p@)) (nClip (Mul byte4! (vstd!arithmetic.power2.pow2.? (I (nClip (Add
