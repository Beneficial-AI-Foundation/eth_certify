...
;; curve25519-dalek/src/lemmas/common_lemmas/pow_lemmas.rs:875:1: 875:86 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::pow_lemmas::lemma_chunk_extraction_commutes_with_mod
;; curve25519-dalek/src/lemmas/common_lemmas/pow_lemmas.rs:875:1: 875:86 (#0)
 ...
 (assert
  (not (and
    (=>
     (= tmp%1 (nClip (Mul k! b!)))
     (=>
      (ens%curve25519_dalek!lemmas.common_lemmas.pow_lemmas.lemma_pow2_div_mod. x! tmp%1
       b!
      )
      (=>
       %%location_label%%0
       (= (EucMod (EucDiv x! (vstd!arithmetic.power2.pow2.? (I (nClip (Mul k! b!))))) (vstd!arithmetic.power2.pow2.?
        ...
        ) (EucDiv (EucMod x! (vstd!arithmetic.power2.pow2.? (I (nClip (Add (nClip (Mul k! b!))
              b!
          ))))
         ) (vstd!arithmetic.power2.pow2.? (I (nClip (Mul k! b!))))
    )))))
    (=>
     (= (EucMod (EucDiv x! (vstd!arithmetic.power2.pow2.? (I (nClip (Mul k! b!))))) (vstd!arithmetic.power2.pow2.?
        (I b!)
     ...
     (=>
      (= y@ (EucMod x! (vstd!arithmetic.power2.pow2.? (I m!))))
      (and
       (=>
        (= tmp%2 (nClip (Mul k! b!)))
        (=>
         (ens%curve25519_dalek!lemmas.common_lemmas.pow_lemmas.lemma_pow2_div_mod. y@ tmp%2
          b!
         )
         (=>
          %%location_label%%1
          (= (EucMod (EucDiv y@ (vstd!arithmetic.power2.pow2.? (I (nClip (Mul k! b!))))) (vstd!arithmetic.power2.pow2.?
             (I b!)
            )
           ) (EucDiv (EucMod y@ (vstd!arithmetic.power2.pow2.? (I (nClip (Add (nClip (Mul k! b!))
                 b!
             ))))
            ) (vstd!arithmetic.power2.pow2.? (I (nClip (Mul k! b!))))
       )))))
          ...
          (and
           (=>
            ...
            (=>
             ...
             (and
              (=>
               (ens%vstd!arithmetic.power2.lemma_pow2_adds. s@ d@)
               (=>
                %%location_label%%2
                (= (vstd!arithmetic.power2.pow2.? (I m!)) (nClip (Mul ps@ pd@)))
              ))
              (=>
               (= (vstd!arithmetic.power2.pow2.? (I m!)) (nClip (Mul ps@ pd@)))
               (and
                (=>
                 ...
                 (=>
                  ...
                  (and
                   (=>
                    %%location_label%%3
                    (req%vstd!arithmetic.div_mod.lemma_mod_mod. x! ps@ pd@)
                   )
                   (=>
                    (ens%vstd!arithmetic.div_mod.lemma_mod_mod. x! ps@ pd@)
                    (=>
                     %%location_label%%4
                     (= (EucMod (EucMod x! (nClip (Mul ps@ pd@))) ps@) (EucMod x! ps@))
                )))))
                (=>
                 (= (EucMod (EucMod x! (nClip (Mul ps@ pd@))) ps@) (EucMod x! ps@))
                 (=>
                  %%location_label%%5
                  (= (EucMod x! ps@) (EucMod y@ ps@))
           )))))))
           (=>
            (= (EucMod x! ps@) (EucMod y@ ps@))
            (=>
