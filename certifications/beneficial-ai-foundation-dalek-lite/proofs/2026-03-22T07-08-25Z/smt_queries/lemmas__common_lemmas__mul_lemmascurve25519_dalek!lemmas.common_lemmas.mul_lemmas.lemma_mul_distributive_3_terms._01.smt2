...
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:61:1: 61:79 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::mul_lemmas::lemma_mul_distributive_3_terms
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:61:1: 61:79 (#0)
...
(push)
 ...
 (assert
  (not (and
    (=>
     (= tmp%1 (Add (Add x1! x2!) x3!))
     (=>
      (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. n! tmp%1)
      (=>
       %%location_label%%0
       (= (Mul n! (Add (Add x1! x2!) x3!)) (Mul (Add (Add x1! x2!) x3!) n!))
    )))
    (=>
     (= (Mul n! (Add (Add x1! x2!) x3!)) (Mul (Add (Add x1! x2!) x3!) n!))
     (and
      (=>
       (= tmp%2 (Add x1! x2!))
       (=>
        (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add. n! tmp%2 x3!)
        (=>
         %%location_label%%1
         (= (Mul n! (Add (Add x1! x2!) x3!)) (Add (Mul n! (Add x1! x2!)) (Mul n! x3!)))
      )))
      (=>
       (= (Mul n! (Add (Add x1! x2!) x3!)) (Add (Mul n! (Add x1! x2!)) (Mul n! x3!)))
       (and
        (=>
         (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add. n! x1! x2!)
         (=>
          %%location_label%%2
          (= (Mul n! (Add x1! x2!)) (Add (Mul n! x1!) (Mul n! x2!)))
        ))
        (=>
         (= (Mul n! (Add x1! x2!)) (Add (Mul n! x1!) (Mul n! x2!)))
         (=>
