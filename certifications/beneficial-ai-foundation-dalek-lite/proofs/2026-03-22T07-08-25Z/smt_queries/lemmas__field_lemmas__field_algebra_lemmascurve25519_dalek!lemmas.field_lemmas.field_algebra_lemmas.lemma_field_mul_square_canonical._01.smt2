...
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2903:1: 2903:62 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_mul_square_canonical
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2903:1: 2903:62 (#0)
    ...
    (=>
     ...
     (=>
      ...
      (and
       (=>
        %%location_label%%0
        (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop_right. v! tmp%1 pn@)
       )
       (=>
        (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop_right. v! tmp%1 pn@)
        (and
         (=>
          (= tmp%2 (nClip (Mul r! r!)))
          (=>
           (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. v! tmp%2)
           (=>
            %%location_label%%1
            (= (nClip (Mul v! (nClip (Mul r! r!)))) (nClip (Mul (nClip (Mul r! r!)) v!)))
         )))
         (=>
          (= (nClip (Mul v! (nClip (Mul r! r!)))) (nClip (Mul (nClip (Mul r! r!)) v!)))
          (=>
           ...
           (and
            (=>
             %%location_label%%2
             (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop_left. tmp%3 v! pn@)
            )
            (=>
             (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop_left. tmp%3 v! pn@)
             (and
              (=>
               (ens%vstd!arithmetic.mul.lemma_mul_is_associative. r! r! v!)
               (=>
                %%location_label%%3
                (= (nClip (Mul (nClip (Mul r! r!)) v!)) (nClip (Mul (nClip (Mul r! r!)) v!)))
              ))
              (=>
               (= (nClip (Mul (nClip (Mul r! r!)) v!)) (nClip (Mul (nClip (Mul r! r!)) v!)))
               (and
