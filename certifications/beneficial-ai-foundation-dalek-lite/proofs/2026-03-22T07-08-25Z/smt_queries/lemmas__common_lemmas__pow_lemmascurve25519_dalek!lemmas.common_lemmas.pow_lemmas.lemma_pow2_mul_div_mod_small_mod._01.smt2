...
;; curve25519-dalek/src/lemmas/common_lemmas/pow_lemmas.rs:777:1: 777:78 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::pow_lemmas::lemma_pow2_mul_div_mod_small_mod
;; curve25519-dalek/src/lemmas/common_lemmas/pow_lemmas.rs:777:1: 777:78 (#0)
    ...
    (and
     ...
     (=>
      ...
      (=>
       ...
       (and
        (=>
         (ens%vstd!arithmetic.power2.lemma_pow2_adds. dd@ t!)
         (=>
          (= tmp%1 (vstd!arithmetic.power2.pow2.? (I dd@)))
          (=>
           (= tmp%2 (vstd!arithmetic.power2.pow2.? (I t!)))
           (=>
            (ens%vstd!arithmetic.mul.lemma_mul_is_associative. x! tmp%1 tmp%2)
            (=>
             %%location_label%%2
             (= (nClip (Mul x! (vstd!arithmetic.power2.pow2.? (I d@)))) (nClip (Mul (nClip (Mul x!
                  (vstd!arithmetic.power2.pow2.? (I dd@))
                 )
                ) (vstd!arithmetic.power2.pow2.? (I t!))
        ))))))))
        (=>
         ...
         (and
          (=>
           ...
           (=>
            %%location_label%%3
            (> (vstd!arithmetic.power2.pow2.? (I t!)) 0)
          ))
          (=>
           (> (vstd!arithmetic.power2.pow2.? (I t!)) 0)
           (and
            (=>
             (= tmp%3 (nClip (Mul x! (vstd!arithmetic.power2.pow2.? (I dd@)))))
             (=>
              (= tmp%4 (vstd!arithmetic.power2.pow2.? (I t!)))
              (and
               (=>
                %%location_label%%4
                (req%vstd!arithmetic.div_mod.lemma_mod_multiples_basic. tmp%3 tmp%4)
               )
               (=>
                (ens%vstd!arithmetic.div_mod.lemma_mod_multiples_basic. tmp%3 tmp%4)
                (=>
                 %%location_label%%5
                 (= (EucMod (nClip (Mul (nClip (Mul x! (vstd!arithmetic.power2.pow2.? (I dd@)))) (vstd!arithmetic.power2.pow2.?
                      (I t!)
                    ))
                   ) (vstd!arithmetic.power2.pow2.? (I t!))
                  ) 0
