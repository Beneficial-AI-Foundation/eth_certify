...
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2354:7: 2354:68 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::field_algebra_lemmas::lemma_foil_sum_reorder
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2354:7: 2354:68 (#0)
  ...
  (not (=>
    ...
    (and
     (=>
      ...
      (=>
       (= tmp%1 (nClip (Add ac! ad!)))
       (=>
        (= tmp%2 (nClip (Add bc! bd!)))
        (and
         (=>
          %%location_label%%0
          (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. tmp%1 tmp%2 p@)
         )
         (=>
          (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. tmp%1 tmp%2 p@)
          (and
           (=>
            %%location_label%%1
            (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. ac! ad! p@)
           )
           (=>
            (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. ac! ad! p@)
            (and
             (=>
              %%location_label%%2
              (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. bc! bd! p@)
             )
             (=>
              (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. bc! bd! p@)
              (=>
               (= tmp%3 (nClip (Add ac! bd!)))
               (=>
                (= tmp%4 (nClip (Add ad! bc!)))
                (and
                 (=>
                  %%location_label%%3
                  (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. tmp%3 tmp%4 p@)
                 )
                 (=>
                  (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. tmp%3 tmp%4 p@)
                  (and
                   (=>
                    %%location_label%%4
                    (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. ac! bd! p@)
                   )
                   (=>
                    (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. ac! bd! p@)
                    (and
                     (=>
                      %%location_label%%5
                      (req%vstd!arithmetic.div_mod.lemma_add_mod_noop. ad! bc! p@)
                     )
                     (=>
                      (ens%vstd!arithmetic.div_mod.lemma_add_mod_noop. ad! bc! p@)
                      (=>
                       (= tmp%5 (= (nClip (Add (nClip (Add (nClip (Add ac! ad!)) bc!)) bd!)) (nClip (Add (nClip
                            (Add (nClip (Add ac! bd!)) ad!)
                           ) bc!
                       ))))
                       (and
                        (=>
