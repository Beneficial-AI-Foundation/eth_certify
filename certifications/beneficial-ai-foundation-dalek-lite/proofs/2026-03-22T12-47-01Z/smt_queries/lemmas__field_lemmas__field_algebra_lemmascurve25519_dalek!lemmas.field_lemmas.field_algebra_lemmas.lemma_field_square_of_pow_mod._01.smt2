...
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2811:1: 2811:59 (#0)
...
;; Function-Def curve25519_dalek::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_square_of_pow_mod
;; curve25519-dalek/src/lemmas/field_lemmas/field_algebra_lemmas.rs:2811:1: 2811:59 (#0)
 ...
 (assert
  (not (=>
    ...
    (=>
     ...
     (=>
      (= xk@ (vstd!arithmetic.power.pow.? (I x!) (I k!)))
      (and
       (=>
        %%location_label%%0
        (req%curve25519_dalek!lemmas.common_lemmas.pow_lemmas.lemma_pow_nonnegative. x! k!)
       )
       (=>
        (ens%curve25519_dalek!lemmas.common_lemmas.pow_lemmas.lemma_pow_nonnegative. x! k!)
        (=>
         (= y@ (EucMod (nClip xk@) pn@))
         (and
          (and
           (=>
            %%location_label%%1
            (req%vstd!arithmetic.div_mod.lemma_mul_mod_noop. (nClip xk@) (nClip xk@) pn@)
           )
           (=>
            (ens%vstd!arithmetic.div_mod.lemma_mul_mod_noop. (nClip xk@) (nClip xk@) pn@)
            (=>
             %%location_label%%2
             (= (curve25519_dalek!specs.field_specs.field_square.? (I y@)) (EucMod (nClip (Mul xk@
                 xk@
                )
               ) pn@
          )))))
          (=>
           ...
           (and
            (=>
             (ens%vstd!arithmetic.power.lemma_pow_multiplies. x! k! 2)
             (=>
              %%location_label%%3
              (= (vstd!arithmetic.power.pow.? (I xk@) (I 2)) (vstd!arithmetic.power.pow.? (I x!)
                (I (nClip (Mul 2 k!)))
            ))))
            (=>
             (= (vstd!arithmetic.power.pow.? (I xk@) (I 2)) (vstd!arithmetic.power.pow.? (I x!)
