...
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:78:1: 78:88 (#0)
...
;; Function-Def curve25519_dalek::lemmas::common_lemmas::mul_lemmas::lemma_mul_distributive_4_terms
;; curve25519-dalek/src/lemmas/common_lemmas/mul_lemmas.rs:78:1: 78:88 (#0)
...
(push)
 ...
 (assert
  (not (and
    (=>
     (= tmp%1 (Add (Add (Add x1! x2!) x3!) x4!))
     (=>
      (ens%vstd!arithmetic.mul.lemma_mul_is_commutative. n! tmp%1)
      (=>
       %%location_label%%0
       (= (Mul n! (Add (Add (Add x1! x2!) x3!) x4!)) (Mul (Add (Add (Add x1! x2!) x3!) x4!)
         n!
    )))))
    (=>
     (= (Mul n! (Add (Add (Add x1! x2!) x3!) x4!)) (Mul (Add (Add (Add x1! x2!) x3!) x4!)
     ...
     (and
      (=>
       (= tmp%2 (Add (Add x1! x2!) x3!))
       (=>
        (ens%vstd!arithmetic.mul.lemma_mul_is_distributive_add. n! tmp%2 x4!)
        (=>
         %%location_label%%1
         (= (Mul n! (Add (Add (Add x1! x2!) x3!) x4!)) (Add (Mul n! (Add (Add x1! x2!) x3!))
           (Mul n! x4!)
      )))))
      (=>
       (= (Mul n! (Add (Add (Add x1! x2!) x3!) x4!)) (Add (Mul n! (Add (Add x1! x2!) x3!))
       ...
       (and
        (=>
         ...
         (=>
          %%location_label%%2
          (= (Mul n! (Add (Add x1! x2!) x3!)) (Add (Add (Mul n! x1!) (Mul n! x2!)) (Mul n! x3!)))
        ))
        (=>
         (= (Mul n! (Add (Add x1! x2!) x3!)) (Add (Add (Mul n! x1!) (Mul n! x2!)) (Mul n! x3!)))
         (=>
